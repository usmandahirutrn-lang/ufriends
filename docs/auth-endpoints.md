...
Me
```bash
curl http://localhost:3000/api/me -H "Authorization: Bearer <ACCESS_JWT>"
```
...
Signup + OTP
```bash
curl -X POST http://localhost:3000/api/auth/signup-otp/request \
  -H "Content-Type: application/json" \
  -d '{"email":"user@example.com","phone":"+2348012345678"}'

curl -X POST http://localhost:3000/api/auth/signup-otp/verify \
  -H "Content-Type: application/json" \
