...
import { addAutomatedService } from "@/lib/automated-services-storage"
import { authFetch } from "@/lib/client-auth"
import jsPDF from "jspdf"

export default function NBAISPage() {
  ...
  // DB-first tiered pricing per unit (fallback to local UI price)
  useEffect(() => {
    ...
    const loadPrice = async () => {
      ...
      try {
        const slug = `education.nbais.${toSlugId(serviceType)}`
        ...
        // API returns top-level numeric `price` and `pricing` object
        if (res.ok && typeof data?.price === "number") {
          setServicePrice(Number(data.price))
        } else {
          const prices = JSON.parse(localStorage.getItem("ufriends_service_prices") || "{}")
          const price = prices[`Education-NBAIS-${serviceType}`]?.sell || 500
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call the API
      const response = await authFetch('/api/service/education/nbais', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          idempotencyKey: `nbais-${Date.now()}-${Math.random()}`
        })
      });

      if (response.ok) {
        const result = await response.json();
        setReferenceId(result.reference || `NBAIS-${Date.now()}`);
        setShowSuccess(true);
        
        toast({
          title: "Success!",
          description: `Your NBAIS Pin(s) have been generated successfully${result.reference ? ` with reference: ${result.reference}` : ''}.`,
        });
      } else {
      ...
      // Also add to local storage for demo continuity
      const request = addAutomatedService({
        userId: "u_001",
  ...
  const generatePDF = () => {
    ...
    doc.text(`Price per Unit: ₦${servicePrice.toLocaleString()}`, 20, 90)
    doc.text(`Total Paid: ₦${totalPrice.toLocaleString()}`, 20, 100)
    doc.text(`Reference ID: ${referenceId}`, 20, 110)
    doc.text(`Date: ${new Date().toLocaleString()}`, 20, 120)

    doc.setFontSize(10)
    doc.setTextColor(100, 100, 100)
    doc.text("Generated by UFriends.com.ng — For Academic Use Only", 105, 280, { align: "center" })

    doc.save("NBAIS_Pin_Receipt.pdf")
  }
        ...
        >
          <Card className="border-[#3457D5]">
            ...
            <CardContent className="pt-6 space-y-4">
              <p className="text-lg">Your NBAIS Pin(s) have been generated successfully.</p>
    ...
    <div className="min-h-screen bg-[#F9F7F3] p-6">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="text-4xl font-bold text-[#3457D5] mb-2">NBAIS Services</h1>
          <p className="text-gray-600">Purchase NBAIS examination pins and scratch cards</p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select value={serviceType} onValueChange={setServiceType} required>
                      ...
                      <SelectContent>
                        <SelectItem value="NBAIS Pin">NBAIS Pin / Scratch Card</SelectItem>
                  ...
                  {serviceType && (
                    <div className="bg-[#CCCCFF]/20 p-4 rounded-lg">
                      <p className="text-sm text-gray-700">
                        💰 Price per unit: ₦{servicePrice.toLocaleString()} — Total: ₦{totalPrice.toLocaleString()}
                      </p>
                    </div>
                  )}
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            <Card className="bg-gradient-to-br from-[#3457D5]/10 to-[#CCCCFF]/20">
              <CardHeader>
                <CardTitle className="text-[#3457D5]">About NBAIS Services</CardTitle>
              </CardHeader>
