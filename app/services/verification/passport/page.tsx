...
export default function PassportVerificationPage() {
  ...
  const { price, isLoading, error: pricingError, submitService } = useDynamicPricing("passport", "verify", "basic")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!passportNumber || !lastName) {
      toast({ title: "Error", description: "Enter passport number and last name", variant: "destructive" })
      return
    }
  ...
  const handlePinConfirm = async (pin: string) => {
    ...
    try {
      ...
      setShowResult(true)
      toast({ title: "Success", description: "Passport verification completed!" })
    } catch (err: any) {
  ...
  const generatePDF = async () => {
    const tplId = process.env.NEXT_PUBLIC_TEMPLATE_ID_PASSPORT_VERIFY
    if (tplId) {
      try {
        await downloadPdfViaServer(tplId, "passport.verify", { passportNumber, lastName }, "passport-verification")
        toast({ title: "Success", description: "Downloaded Passport slip" })
        return
      } catch (err: any) {
    ...
    try {
      await downloadPdfAuto("passport.verify", { passportNumber, lastName }, "passport-verification")
      toast({ title: "Success", description: "Downloaded Passport slip" })
      return
    } catch { }
    const data: any = apiResult?.data ?? (apiResult as any)?.result ?? {}
    const number = data.passport_number || passportNumber
    const surname = data.surname || lastName
    const givenNames = data.given_names || data.first_name || ""

    const doc = new jsPDF()
    doc.setFontSize(18)
    doc.text("UFriends Information Technology", 70, 15)
    doc.setFontSize(14)
    doc.text("International Passport Verification Slip", 60, 25)
    doc.setFontSize(10)
    doc.text(`Date: ${new Date().toLocaleString()}`, 15, 40)
    doc.text(`Passport Number: ${number}`, 15, 50)
    if (surname) doc.text(`Surname: ${surname}`, 15, 60)
    if (givenNames) doc.text(`Given Names: ${givenNames}`, 15, 70)
    doc.text(`Price: ₦${price ?? 500}`, 15, 80)
    doc.text("Status: Verified", 15, 90)
    doc.text("Note: This is a verification slip generated by UFriends.com.ng", 15, 250)
    doc.save(`passport-verification-${Date.now()}.pdf`)
  }
            ...
            >
              <Card className="shadow-lg border-[#CCCCFF]/30">
                <CardContent className="p-6 md:p-8">
                  <h1 className="text-2xl font-bold mb-2 text-gray-900">International Passport Verification</h1>
                  <p className="text-sm text-gray-600 mb-8">Verify Nigerian international passport details</p>
                  ...
                  <form className="space-y-6" onSubmit={handleSubmit}>
                    ...
                    >
                      {isVerifying ? "Verifying..." : `Verify - ₦${price ?? 500}`}
                    </Button>
                  </form>
                </CardContent>
              </Card>
            ...
            <motion.div key="result" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <Card className="shadow-lg border-[#CCCCFF]/30">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-gray-600 mb-6">Passport has been successfully verified.</p>
                  {(() => {
                    const data: any = apiResult?.data ?? (apiResult as any)?.result ?? {}
                    const number = data.passport_number || passportNumber
                    const surname = data.surname || lastName
                    ...
                    return (
                      <div className="text-left max-w-md mx-auto mb-6 text-sm text-gray-700">
                        <div className="flex justify-between"><span className="font-medium">Passport No.</span><span>{number}</span></div>
                        {surname && <div className="flex justify-between"><span className="font-medium">Surname</span><span>{surname}</span></div>}
                        {givenNames && <div className="flex justify-between"><span className="font-medium">Given Names</span><span>{givenNames}</span></div>}
                      </div>
                    )
                  })()}
