...
export default function PhoneVerificationPage() {
  ...
  const { price: servicePrice, isLoading, error: pricingError, submitService } = useDynamicPricing("phone", "advanced", "basic")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (phone.length !== 11 || !phone.startsWith("0")) {
      toast({ title: "Error", description: "Phone number must be 11 digits and start with 0", variant: "destructive" })
      return
    }

    setPendingPayload({
      amount: Number(servicePrice || 300),
  ...
  const handlePinConfirm = async (pin: string) => {
    ...
    try {
      ...
      setShowResult(true)
      toast({ title: "Success", description: "Phone number verification completed!" })
    } catch (err: any) {
  ...
  const generatePDF = async () => {
    const tplId = process.env.NEXT_PUBLIC_TEMPLATE_ID_PHONE_VERIFY_ADVANCED
    if (tplId) {
      try {
        await downloadPdfViaServer(tplId, "phone.verify_advanced", { phoneNumber: phone }, "phone-verification")
        toast({ title: "Success", description: "Downloaded Phone slip" })
        return
      } catch (err: any) {
    ...
    try {
      await downloadPdfAuto("phone.verify_advanced", { phoneNumber: phone }, "phone-verification")
      toast({ title: "Success", description: "Downloaded Phone slip" })
      return
    } catch { }
    const data: any = apiResult?.data ?? (apiResult as any)?.result ?? {}
    const owner = data.name || data.full_name || data.ownerName || data.owner || ""
    const number = data.phoneNumber || data.phone || phone

    const doc = new jsPDF()
    doc.setFontSize(18)
    doc.text("UFriends Information Technology", 70, 15)
    doc.setFontSize(14)
    doc.text("Phone Number Verification Slip", 70, 25)
    doc.setFontSize(10)
    doc.text(`Date: ${new Date().toLocaleString()}`, 15, 40)
    doc.text(`Phone Number: ${number}`, 15, 50)
    if (owner) doc.text(`Owner: ${owner}`, 15, 60)
    doc.text(`Price: ₦${servicePrice}`, 15, 70)
    doc.text("Status: Verified", 15, 80)
    doc.text("Note: This is a verification slip generated by UFriends.com.ng", 15, 250)
    doc.save(`phone-verification-${Date.now()}.pdf`)
  }
            ...
            >
              <Card className="shadow-lg border-[#CCCCFF]/30">
                <CardContent className="p-6 md:p-8">
                  <h1 className="text-2xl font-bold mb-2 text-gray-900">Phone Number Verification</h1>
                  <p className="text-sm text-gray-600 mb-8">Verify phone number registration and owner details</p>
                  ...
                  <form className="space-y-6" onSubmit={handleSubmit}>
                    ...
                    >
                      {isVerifying ? "Verifying..." : (isLoading ? "Fetching price..." : `Verify - ₦${servicePrice ?? 300}`)}
                    </Button>
                  </form>
                </CardContent>
              </Card>
            ...
            <motion.div key="result" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <Card className="shadow-lg border-[#CCCCFF]/30">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-gray-600 mb-6">Phone number has been successfully verified.</p>
                  {(() => {
                    const data: any = apiResult?.data ?? (apiResult as any)?.result ?? {}
                    const owner = data.name || data.full_name || data.ownerName || data.owner
                    const number = data.phoneNumber || data.phone || phone
                    return (
                      <div className="text-left max-w-md mx-auto mb-6 text-sm text-gray-700">
                        <div className="flex justify-between"><span className="font-medium">Phone</span><span>{number}</span></div>
                        {owner && <div className="flex justify-between"><span className="font-medium">Owner</span><span>{owner}</span></div>}
                      </div>
                    )
                  })()}
