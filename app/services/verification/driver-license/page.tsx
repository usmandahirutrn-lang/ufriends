...
export default function DriverLicenseVerificationPage() {
  ...
  const lookupMode = licenseNumber ? "number" : (firstName && lastName && dob ? "name_dob" : "unknown")
  const { price: servicePrice, isLoading, error: pricingError, submitService } = useDynamicPricing("driver-license", "basic", "basic", { lookupMode })
  ...
  // pricing now handled by useDynamicPricing
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!licenseNumber && !(firstName && lastName && dob)) {
      toast({ title: "Error", description: "Provide license number or full name + dob", variant: "destructive" })
      return
    }
  ...
  const handlePinConfirm = async (pin: string) => {
    ...
    try {
      ...
      setShowResult(true)
      toast({ title: "Success", description: "Driver's License verification completed!" })
    } catch (err: any) {
  ...
  const generatePDF = async () => {
    const tplId = process.env.NEXT_PUBLIC_TEMPLATE_ID_DRIVERS_LICENSE_VERIFY
    if (tplId) {
      try {
        await downloadPdfViaServer(tplId, "drivers_license.verify", { licenseNumber, firstName, lastName, dob }, "drivers-license-verification")
        toast({ title: "Success", description: "Downloaded Driver's License slip" })
        return
      } catch (err: any) {
    ...
    try {
      await downloadPdfAuto("drivers_license.verify", { licenseNumber, firstName, lastName, dob }, "drivers-license-verification")
      toast({ title: "Success", description: "Downloaded Driver's License slip" })
      return
    } catch { }
    const data: any = apiResult?.data ?? (apiResult as any)?.result ?? {}
    const license = data.licenseNumber || licenseNumber
    const fullName = data.full_name || `${data.first_name || ""} ${data.last_name || ""} `.trim() || `${firstName} ${lastName} `
    const dobStr = data.dob || data.date_of_birth || dob

    const doc = new jsPDF()
    doc.setFontSize(18)
    doc.text("UFriends Information Technology", 70, 15)
    doc.setFontSize(14)
    doc.text("Driver's License Verification Slip", 70, 25)
    doc.setFontSize(10)
    doc.text(`Date: ${new Date().toLocaleString()} `, 15, 40)
    doc.text(`License Number: ${license} `, 15, 50)
    if (fullName) doc.text(`Name: ${fullName} `, 15, 60)
    if (dobStr) doc.text(`DOB: ${dobStr} `, 15, 70)
    ...
    doc.save(`driver - license - verification - ${Date.now()}.pdf`)
  }
            ...
            >
              <Card className="shadow-lg border-[#CCCCFF]/30">
                <CardContent className="p-6 md:p-8">
                  <h1 className="text-2xl font-bold mb-2 text-gray-900">Driver's License Verification</h1>
                  <p className="text-sm text-gray-600 mb-8">Verify driver's license details with FRSC database</p>
                  ...
                  <form className="space-y-6" onSubmit={handleSubmit}>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Provide either license number OR full name and date of birth.
                    </p>
                    ...
                    <Button
                      ...
                      disabled={!consentChecked || !termsAccepted || isVerifying}
                    >
                      {isVerifying ? "Verifying..." : `Verify - ₦${servicePrice ?? 500} `}
                    </Button>
                  </form>
                </CardContent>
              </Card>
            ...
            <motion.div key="result" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <Card className="shadow-lg border-[#CCCCFF]/30">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-gray-600 mb-6">Driver's License has been successfully verified.</p>
                  {(() => {
                    const data: any = apiResult?.data ?? (apiResult as any)?.result ?? {}
                    const license = data.licenseNumber || licenseNumber
                    const fullName = data.full_name || `${data.first_name || ""} ${data.last_name || ""} `.trim() || `${firstName} ${lastName} `
                    ...
                    return (
                      <div className="text-left max-w-md mx-auto mb-6 text-sm text-gray-700">
                        <div className="flex justify-between"><span className="font-medium">License No.</span><span>{license}</span></div>
                        {fullName && <div className="flex justify-between"><span className="font-medium">Name</span><span>{fullName}</span></div>}
                        {dobStr && <div className="flex justify-between"><span className="font-medium">DOB</span><span>{dobStr}</span></div>}
                      </div>
                    )
                  })()}
