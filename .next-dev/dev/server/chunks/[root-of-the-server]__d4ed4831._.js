...
"[project]/app/api/admin/reports/metrics/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(req) {
    try {
        ...
        for (const tx of txs){
            ...
            monthlyActiveUsers[mk.key] = monthlyActiveUsers[mk.key] || new Set();
            ...
            // User aggregates
            userAgg[tx.userId] = userAgg[tx.userId] || {
                userId: tx.userId,
                revenue: 0,
                requests: 0
            };
            ...
            if (tx.status === "SUCCESS") {
                ...
                revenueBuckets[mk.key] = revenueBuckets[mk.key] || {
                    month: mk.label,
                    revenue: 0,
                    profit: 0
                };
        ...
        const topServices = Object.values(svcAgg).sort((a, b)=>b.revenue - a.revenue).slice(0, topLimit);
        const topUserIds = Object.keys(userAgg).sort((a, b)=>userAgg[b].revenue - userAgg[a].revenue).slice(0, topLimit);
        ...
        const userGrowth = Object.entries(monthlyActiveUsers).map(([key, set])=>{
            const [y, m] = key.split("-");
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d4ed4831._.js.map